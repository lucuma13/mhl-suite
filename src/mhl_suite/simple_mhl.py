...
# =============================================================================
# This module is the command-line front end. The hashing, sealing, and
# verification logic lives in mhl_suite.classicmhl / mhl_suite.shared; simple_mhl wires argparse up to it
# and renders the engine's structured results to the terminal. mhlver drives the
# same engine in-process (its ClassicMHLBackend) rather than shelling out.
#
...
#
# Many engine internals are re-exported here (see __all__) because the test
# suite references and monkeypatches them via the simple_mhl namespace.
# =============================================================================
...
from lxml import etree  # noqa: F401 — re-exported so tests can patch simple_mhl.etree.* (shared module)

from mhl_suite.classicmhl.seal import (
    _build_creatorinfo,
    _iter_files_for_seal,
    _reject_empty_files,
    _resolve_output_dir,
    _resolve_seal_algorithms,
    seal,
)
from mhl_suite.classicmhl.verify import (
    _CANONICAL_TAGS,
    _VERIFY_ALL,
    _free_processed,
    _is_newer_than_classic,
    _localname,
    _make_ci_matcher,
    _manifest_tag_of,
    _matches_be,
...
from mhl_suite.shared.hashing import (
    _NULL_RANK,
    _NULL_TAG,
    ALGO_MAP,
    HASH_CHUNK_SIZE,
    _calibrate_hash_bw,
    _calibrate_hash_bw_multi,
    _hash_batch,
    _hash_files_auto,
    _hash_jobs_auto,
    _Hasher,
...
# Re-exports for the test suite and any external caller poking at internals.
__all__ = [
    "ALGO_MAP",
    "HASH_CHUNK_SIZE",
    "_CANONICAL_TAGS",
...
def verify(
    ...
    Thin wrapper over classicmhl.verify.verify_manifest: validates the path, rejects
    ...
    Exit codes: 0 clean, 1 not an MHL file, 2 bad algorithm, 20 malformed XML,
    30 missing files only, 40 hash/size mismatches only, 70 both.
    """
    _validate_mhl_path(mhl_file)
    _reject_ascmhl_v2(mhl_file)
    try:
        report = verify_manifest(mhl_file, algorithm=algorithm, size_only=size_only)
...
# -----------------------------------------------------------------------------
# XSD schema validation (CLI wrapper)
# -----------------------------------------------------------------------------
