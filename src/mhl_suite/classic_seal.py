...
Walks a directory, hashes every non-excluded file, and writes a classic MHL
manifest. The hashing controller lives in mhl_suite.hashing and is reached
through the module object so test monkeypatches still intercept it.
"""
...
def _iter_files_for_seal(
    ...
    Implementation note: we use os.scandir rather than os.walk because DirEntry
    caches the stat() info from getdents64, saving a syscall per file. Bench
    showed ~13% faster traversal on a 1000-file tree. The code is structured
    iteratively (stack-based) rather than recursively to avoid Python's
    ...
    # same order. The overall manifest is NOT globally sorted by full path — a
    # deeply nested a/b/c.mov appears before a sibling d.mov at the parent
    ...
    while pending:
        current = pending.pop()
        try:
            # scandir returns DirEntry objects whose .stat() is satisfied from
            # the readdir cache (no extra syscall) on most filesystems.
...
def _resolve_seal_algorithms(algorithms: "list[str]") -> list[str]:
    ...
    called directly, so we re-validate here and exit 2 on an unknown algorithm.
    Aliases resolving to the same manifest tag (e.g. xxhash/xxh64/xxhash64be)
    are deduped, preserving first-appearance order, so each tag is emitted at
    most once.
...
def _resolve_output_dir(root: str, output_dir: "str | None") -> str:
    """
    Resolve and validate where seal writes the manifest, exiting 2 on a bad
    choice.
    ...
    ancestor keeps the recorded paths traversal-free. Returns the absolute
    directory.
    """
...
def _reject_empty_files(entries: "list[tuple[str, os.stat_result]]", root: str, fd: int, mhl_path: str) -> None:
    ...
    file (zero bytes) must not be represented in any seal mode. The scan is a
    pure in-memory pass over the stat info the walk already collected (no extra
    syscalls), running before any hashing so no compute is wasted. On abort it
    closes + unlinks the O_EXCL manifest the caller already claimed.
    """
...
def seal_classic(root: str, algorithms: "list[str]", verbose: bool = False, output_dir: "str | None" = None) -> None:
    ...
    verify resolves each <file> entry relative to the manifest's own directory
    and rejects any entry that escapes it via '..', so only an ancestor keeps
    every recorded path traversal-free. The <file> paths are therefore written
    relative to `output_dir`, gaining a leading "<sealed-folder>/…" prefix when
    it is a true ancestor.
    ...
    Behaviour:
      ...
      * Hidden files and directories are included; only OS-generated metadata
        ...
        other volume-internal items — see mhl_suite.ignorelist) is skipped.
    ...
    #
    # We use O_CREAT | O_EXCL so the existence check and file creation are a
    # single syscall — eliminating the TOCTOU
    # race that would exist if we checked os.path.exists() and then separately
    # opened the file for writing.  Two parallel seal invocations (e.g. from
    # concurrent post- transfer scripts on a shared NAS) will now always land on
    # distinct paths; the second process to attempt any given path gets
    # FileExistsError and moves on to the next suffix.
    ...
    # Build the manifest skeleton. We construct in memory and write at the end;
    # for typical shoots (<5000 files) the in-memory tree is a few MB of XML
    # which is fine. For pathologically large trees a streaming writer
    # (etree.xmlfile) would help, but the seal-time cost is dominated by hashing
    # the bytes, not by holding the tree in RAM.
    ...
    # xml_tags name the manifest element each digest is written into, in the
    # same order _hash_files_auto returns them.
    ...
    # a few tens of MB even for a 100k-frame shoot, dwarfed by the XML tree
    # built alongside it. Concurrency is decided entirely by _hash_files_auto,
    # which measures the storage and stays sequential whenever that's faster.
    entries = list(_iter_files_for_seal(root, mhl_path, on_skip=_on_skip))

    # Refuse zero-byte files before any hashing (see _reject_empty_files); on
    # abort it releases the O_EXCL manifest fd we claimed above.
    _reject_empty_files(entries, root, fd, mhl_path)

    paths = [fp for fp, _ in entries]
    # `null` records no digest, so a null seal reads no bytes — each entry just
    # gets an empty <null> and the run timestamp. Any other selection hashes for
    # real, auto-tuning concurrency to the storage.
    ...
    for (filepath, stat_result), (file_digests, hashdate) in zip(entries, digests, strict=True):
        ...
        # codepoints (e.g. é = U+00E9) rather than the NFD decomposed form (e =
        # U+0065 + combining acute U+0301) that macOS HFS+/APFS returns from the
        # filesystem. A consistent NFC manifest round-trips correctly on Linux
        # (byte-exact path matching) and Windows, both of which use NFC
        # natively.
    ...
    # Pretty-print so a human can read the manifest in a text editor; lxml adds
    # the XML declaration and UTF-8 encoding header. We write via the fd we
    # already hold from the O_EXCL open above so we never re-open the file by
    # path (which would be another race window).
