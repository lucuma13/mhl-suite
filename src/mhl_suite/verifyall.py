...
from mhl_suite.classic_verify import is_ascmhl_v2, render_verify_lines, schema_report, verify_classic
from mhl_suite.hashing import ALGO_MAP
from mhl_suite.report import FileResult, ManifestResult
from mhl_suite.verify_results import VerifyReport


def _routes_to_ascmhl_backend(mhl_file: Path) -> bool:
    """
...
_ASCMHL_VERIFY_RESULTS: dict[int, tuple[str, str]] = {
    ...
    # Size-only malformed manifest XML (the raw size parser couldn't read it).
    ExitCode.MALFORMED_XML: ("🚨 Malformed XML: {target} cannot be parsed.", "warning"),
}
...
class AscMHLBackend:
    """Verifies ASC-MHL in-process via mhl_suite.ascmhl_verify."""
...
def _verify_classicmhl(
    ...
    report = verify_classic(str(target), size_only=size_only, on_progress=on_bytes)

    if report.malformed:
        # Malformed XML: the engine read nothing and produced no per-file detail.
        template, _sev = _CLASSICMHL_RESULTS[ExitCode.MALFORMED_XML]
        _emit(emit, _CLASSICMHL_RESULTS, ExitCode.MALFORMED_XML, target.name, "")
        mr = ManifestResult(
    ...
    # VerifyEntry and FileResult share the same fields — a direct, parse-free map.
    file_results = [
        FileResult(
            path=e.path,
            status=e.status,
...
def _verify_ascmhl(
    ...
    """Verify an ASC-MHL package in-process via mhl_suite.ascmhl_verify, by mode."""
    if schema:
...
def _manifest_weights(mhl_files: "list[Path]", size_only: bool) -> "dict[Path, int]":
    ...
    return {f: (_ascmhl_total_bytes(f) if _routes_to_ascmhl_backend(f) else _mhl_total_bytes(f)) for f in mhl_files}
