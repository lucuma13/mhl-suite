...
# VENDORED PATCH (not in upstream): drive ASC-MHL file hashing through the
# suite's adaptive parallel controller (shared.hashing) and expose per-file
# progress + structured results, replacing upstream's inline single-threaded
# hash_file loop in verify_entire_folder. See mhl_suite/ascmhl/verify.py.
from mhl_suite.shared import hashing as _shared_hashing

from . import errors, hasher, ignore, logger, utils
...
from .hasher import DirectoryHashContext, hash_file, multiple_format_hash_file, new_hasher_for_hash_type
from .hashlist import MHLAuthor, MHLCreatorInfo, MHLMediaHash, MHLProcess, MHLProcessInfo, MHLTool
    ...
    for folder_path, children in post_order_lexicographic(root_path, ignore_spec.get_path_spec()):
        ...
        if not no_directory_hashes:
            for hash_format, dir_hash_context in dir_hash_context_lookup.items():
                ...
                if dir_content_hash_mapping_lookup and folder_path in dir_content_hash_mapping_lookup:
...
def _calibrate_ascmhl_hash_bw(hash_format):
    """In-RAM hashing throughput (bytes/sec) for an ASC-MHL hash format.

    VENDORED PATCH (not in upstream): the adaptive parallel controller in
    shared.hashing needs the pure-CPU hash bandwidth to decide whether the disk
    can outrun a single hash thread. shared.hashing only knows md5/sha1/xxh64;
    ASC-MHL also uses xxh128/xxh3/c4, so this calibrates against upstream's own
    incremental hasher. Mirrors shared.hashing._calibrate_hash_bw.
    """
    import time

    buf = bytes(_shared_hashing.HASH_CHUNK_SIZE)
    hasher_obj = new_hasher_for_hash_type(hash_format)
    done = 0
...
def verify_entire_folder(
    ...
    *,
    ...
    VENDORED PATCH (not in upstream): hashing is split into a traversal pass that
    only collects the files to hash and a second pass that runs them through the
    suite's adaptive parallel controller (shared.hashing), so ASC-MHL verify gets
    the same auto-tuned concurrency as classic MHL. Two optional keyword args are
    added (defaults preserve upstream behaviour exactly): `on_progress(nbytes)` is
    called as each file finishes hashing (drives mhlver's per-file progress bar),
    and `report`, when given a list, is filled with shared.results.FileOutcome
    entries so mhlver can build its structured report without scraping logger
    output. The emitted logger lines and the raised exceptions / exit codes are
    unchanged.
    """
    ...
    # PASS 1 — traverse and resolve. New-file detection (which reads no bytes) is
    # handled inline; files that need a hash are deferred to PASS 2 so they can be
    # hashed concurrently. `deferred` keeps traversal (post-order lexicographic)
    # order, so the PASS 2 emit order matches upstream's inline order.
    deferred = []  # list of (file_path, relative_path, original_hash_entry, size)
    for folder_path, children in post_order_lexicographic(root_path, ignore_spec.get_path_spec()):
        for item_name, is_dir in children:
            file_path = os.path.join(folder_path, item_name)
            not_found_paths.discard(file_path)
            ...
            if single_file is None or os.path.realpath(single_file) == os.path.realpath(file_path):
                ...
                if original_hash_entry is None:
                    ...
                    if report is not None:
                        report.append(
                            FileOutcome(path=relative_path, status="new", line=f"found new file {relative_path}")
                        )
                    continue
                ...
                try:
                    ...
                    size = 0
                deferred.append((file_path, relative_path, original_hash_entry, size))
                found_single_file = True

    # PASS 2 — hash the deferred files through the adaptive parallel controller,
    # emitting results (and progress / structured outcomes) in traversal order.
    if deferred:
        paths = [d[0] for d in deferred]
        sizes = [d[3] for d in deferred]
        calib_format = deferred[0][2].hash_format

        def _make_job(fp, entry):
            return lambda: hash_file(fp, entry.hash_format)

        jobs = [_make_job(fp, entry) for fp, _rel, entry, _sz in deferred]
        hashed = _shared_hashing._hash_jobs_auto(paths, sizes, jobs, lambda: _calibrate_ascmhl_hash_bw(calib_format))
        for (file_path, relative_path, original_hash_entry, size), current_hash in zip(deferred, hashed):
                ...
                if report is not None:
                    report.append(
                        FileOutcome(
                            path=relative_path,
                            status="ok",
                            line=f"verification ({original_hash_entry.hash_format}) of file {relative_path}: OK",
                        )
                    )
            else:
                logger.error(
                ...
                if report is not None:
                    ...
                    report.append(
                        FileOutcome(
                            ...
                            status="mismatch",
                            detail=(
                                f"hash mismatch: calc {fmt}: {current_hash} "
                                f"| stored {fmt}: {original_hash_entry.hash_string}"
                            ),
                            line=f"ERROR: hash mismatch for {relative_path}",
                        )
                    )
            if on_progress is not None:
                on_progress(size)

    # Record the still-missing files in the structured report (the logger lines
    # are emitted by test_for_missing_files below). Mirror its ignore filtering.
    if report is not None:
        ...
        for path in not_found_paths:
            ...
            report.append(FileOutcome(path=rel, status="missing", line=f"missing file: {rel}"))
...
def verify_directory_hash_subcommand(
    ...
    if hash_format is None:
        ...
        if not hash_formats:
            ...
            logger.verbose("default hash format: c4")
        else:
    ...
    def add_detected_failure_for_format(failed_hash_format):
        ...
        if failures_per_format_lookup:
            if failed_hash_format in failures_per_format_lookup:
            ...
            for hash_entry in media_hash.hash_entries:
                if logger.verbose_logging == True:
                    ...
                    if media_hash.previous_path and relative_path == media_hash.path:
                        logger.info(f" In previous generations the file was named: {media_hash.previous_path}\n\n")
...
def _bundled_xsd_path(name: str) -> str:
    """Resolve a bundled ASC-MHL schema file shipped in the mhl_suite.xsd package.

    VENDORED PATCH (not in upstream). Looks first at the installed-package
    location via importlib.resources, then falls back to the source-checkout
    xsd/ directory, matching classicmhl.verify.get_xsd_path. Raises
    FileNotFoundError if the schema cannot be located.
    """
    try:
        resource = importlib.resources.files("mhl_suite.xsd").joinpath(name)
        if resource.is_file():
            return str(resource)
    except (ImportError, FileNotFoundError, TypeError):
        pass
    # This file lives at mhl_suite/ascmhl/vendor/commands.py, so the bundled
    # xsd/ package is three parents up (vendor -> ascmhl -> mhl_suite).
    local = Path(__file__).resolve().parent.parent.parent / "xsd" / name
    if local.exists():
        return str(local)
    raise FileNotFoundError(f"Could not locate bundled schema {name} (tried {local})")
...
def xsd_schema_check(file_path, directory_file, xsd_file):
    ...
    # VENDORED PATCH: upstream loads the XSD relative to the current working
    # directory ("xsd/ASCMHL.xsd"). In the suite the schemas are bundled inside
    # the mhl_suite.xsd package, so resolve them there instead (mirrors
    # classicmhl.verify.get_xsd_path). An explicit --xsd_file still wins.
    xsd_path = _bundled_xsd_path("ASCMHL.xsd")

    if directory_file:
        xsd_path = _bundled_xsd_path("ASCMHLDirectory__combined.xsd")
...
def get_ignore_spec_including_nested_ignores(existing_history, ignore_list, ignore_spec_file=None):
    ...
    patterns_from_file = []
    if ignore_spec_file:
        with open(ignore_spec_file) as fh:
    ...
    for pattern in ignore_patterns_cumulated:
        if pattern in ignore.default_ignore_list():
            ...
            if pattern.startswith("/"):
                ...
                pattern.startswith("**")
                or (pattern.endswith("/") and pattern[:-1].find("/") == -1)
                or (pattern.endswith("/" + "**") and pattern[:-3].find("/") == -1)
            ):
                absolute_ignore_paths.append(pattern)
            else:
