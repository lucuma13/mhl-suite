...
from . import chain_xml_parser, ignore, logger
from .hashlist import MHLCreatorInfo, MHLHashEntry, MHLHashList, MHLProcessInfo
    ...
    def get_relevant_ignore_pattern(self, history) -> MHLIgnoreSpec:
        ...
        if history.parent_history is not None:
            ...
            for pattern in ignore_patterns:
                if pattern not in final_ignores:
            ...
            for pattern in ignore_patterns:
                if pattern not in final_ignores:
                    if (
                        ...
                        and pattern not in ignore.default_ignore_list()
                    ):
    ...
    if pattern_rel_path is not None:
        ...
        if "/" in pattern[:-1]:
            return (
                "/" + pattern
                if pattern.startswith("**/")
                else pattern_rel_path
                if pattern_rel_path.startswith("/")
                else "/" + pattern_rel_path
            )
