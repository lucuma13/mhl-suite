...
from . import chain_xml_parser, errors, hasher, hashlist_xml_parser, ignore, logger
from .__version__ import ascmhl_chainfile_name, ascmhl_collectionfile_name, ascmhl_file_extension, ascmhl_folder_name
from .chain import MHLChain
from .hashlist import MHLHashEntry, MHLHashList
...
class MHLHistory:
    ...
    def latest_ignore_patterns(self) -> list[str] | None:
        """
    ...
    def latest_ignore_pattern_from_nested_histories(self) -> list[str] | None:
    ...
    def find_original_hash_entry_for_path(self, relative_path: str) -> MHLHashEntry | None:
    ...
    def find_first_hash_entry_for_path(self, relative_path, hash_format=None) -> MHLHashEntry | None:
        ...
        for hash_list in self.hash_lists:
            ...
            if media_hash is None:
                continue
            for hash_entry in media_hash.hash_entries:
                if (hash_format is not None and hash_entry.hash_format == hash_format) or hash_format is None:
                    return hash_entry
        return None
    ...
    def hash_list_with_file_name(self, file_name) -> MHLHashList | None:
        for hash_list in self.hash_lists:
    ...
    def log(self):
        ...
        logger.info(f"asc_mhl path: {self.asc_mhl_path}")
