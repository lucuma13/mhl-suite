"""
ASC-MHL seal engine and manifest/chain writers.

One operation serves both `seal` and `verify`: hash every file in scope
...
Nested histories are sealed bottom-up in the same operation: every manifest
...
# -----------------------------------------------------------------------------
# The seal/verify engine
# -----------------------------------------------------------------------------


@dataclass(frozen=True)
class SealOptions:
    """What a seal/verify-append operation computes and records."""
...
def _scan_level(prefix: str, history: History, options: SealOptions) -> _Level:
...
def _hash_all(levels: "list[_Level]", options: SealOptions, on_progress: "Callable[[int], None] | None") -> None:
    """
...
def _directory_hashes(
    level: _Level, options: SealOptions, records: "list[DirEntryOut]", notices: "list[str]"
...
def _write_generation(level: _Level, manifest: ManifestOut, op_start: datetime, result: SealResult) -> None:
...
def _assemble_level(
    level: _Level, options: SealOptions, op_start: datetime, result: SealResult, entries: "list[VerifyEntry]"
) -> bool:
    """Seal one level bottom-up (children first); returns whether any directory hash failed."""
    dir_failed = False
    for child in level.children:
...
def seal_ascmhl(
    ...
    *,
    ...
    """
    Seal (or verify-append) the directory `root` as an ASC-MHL managed data
    ...
    """
    options = options or SealOptions()
    algorithms = _resolve_algorithms(options.algorithms)
    options = SealOptions(algorithms, options.directory_hashes, tuple(options.ignore_patterns))
    ...
    result = SealResult(report=VerifyReport(), seal_code=ExitCode.OK)
    ...
    result.seal_code = _seal_exit_code(entries, dir_failed=dir_failed)
    result.report.notices.extend(f"[WARNING] {failure}" for failure in result.write_failures)
    return result


def _seal_exit_code(entries: "list[VerifyEntry]", *, dir_failed: bool) -> ExitCode:
    """
    The seal-semantics exit code: recording new files is the operation's
    purpose, so Status.NEW never fails a seal; failures and absences do.
    """
