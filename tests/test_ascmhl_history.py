...
ASC MHL specification, on two kinds of fixture: packages generated by the
...
covering spec-conformant XML no generator here can produce):
...
class TestVerifyPackage:
    ...
    def test_recorded_ignore_patterns_are_honoured(self, tmp_path):
        """A pattern recorded at generate time keeps its matches out of new-file detection."""
        pkg = tmp_path / "pkg"
        pkg.mkdir()
...
class TestSpecConformance:
    ...
    as missing action attributes, neither of which a generator here can produce.
    """
...
class TestSizeOnlyRecordedSet:
    ...
    def test_missing_recorded_size_is_ok_not_a_failure(self, tmp_path):
        ...
        # 0-byte file (as the fixture generator does here). Absence carries no
    ...
    def test_nested_child_history_files_are_covered(self, tmp_path):
        # A sub-folder generated as its own ASC-MHL history, then wrapped by a
        # parent generate. The parent's size verify must descend into the child,
    ...
    def test_original_generation_size_wins(self, tmp_path):
        # Generate a.mov at 100 bytes, then grow it to 200 and re-generate (creating gen
