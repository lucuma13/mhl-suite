"""
ASC-MHL verification: the in-process wrapper and its size-check internals (mhl_suite.ascmhl_verify).

Three layers, top to bottom:
  - verify_ascmhl / schema_check — the library boundary mhlver drives, across clean and every failure mode, asserting on
    the structured VerifyReport. They prove our own verify loop maps the ASC-MHL exit codes correctly.
  - verify_ascmhl(size_only=True) — the suite's size-only extension (ascmhl has no size check), with its own exit code
    13 for a size mismatch, kept distinct from hash mismatch 11 so a size failure never masquerades as a hash failure.
  - verify_ascmhl_sizes — the size engine, driven off the same loaded MHLHistory the hash path uses. Exercised against
    real sealed packages (plus a couple of targeted stubs for branches a clean package can't carry).
"""
...
def sizes_for(package):
    """{path: SizeCheckResult} from the size engine run on a real, loadable package (its load is the integrity gate)."""
...
class TestCalibrateHashBw:
    """
    The in-RAM hash-bandwidth calibration feeding the adaptive controller for ASC-MHL formats the shared hasher registry
    doesn't know (xxh128/xxh3/c4).
    """

    def test_returns_positive_finite_bandwidth(self):
        # xxh64 is always available; the calibration runs the library's own incremental hasher, so any supported format
        # gives a usable bytes/sec.
        bw = verify._calibrate_hash_bw("xxh64")
        assert 0 < bw < float("inf")


class TestVerifyPackage:
    """verify_ascmhl returns the upstream exit code and structured per-file outcomes."""
    ...
    def test_report_paths_use_forward_slashes(self, package):
        """Nested per-file entries carry the canonical forward-slash path, never the native separator."""
        report = verify.verify_ascmhl(package)
    ...
    def test_relative_root_is_resolved_against_cwd(self, package, monkeypatch):
        """A relative root path is resolved against the current directory before the
        history loads, so verify works whether the caller passes an absolute or relative package path."""
...
class TestSizeOnlyVerify:
    """verify_ascmhl(size_only=True) compares recorded sizes to disk, reading no bytes."""
    ...
    def test_size_change_is_exit_13_not_11(self, package):
        # Append bytes: the file's hash AND size now differ, but size-only reports the size mismatch (13) without ever
        # hashing — distinct from hash 11.
    ...
    def test_bare_reason_mismatch_renders_label_only(self, package, monkeypatch):
        """
        A size failure with no parenthetical detail (e.g. 'blocked traversal attempt') renders as a bare '[ERROR]
        <reason>: <path>' with no continuation line. The real package passes the integrity gate so the wrapper reaches
        this mapping; the size result is stubbed since a clean sealed package can't carry an escaping path.
        """
        monkeypatch.setattr(
            verify,
            "verify_ascmhl_sizes",
    ...
    def test_integrity_gate_failure_passes_through(self, tmp_path):
        # No ASC-MHL history at all: the history load (which doubles as the integrity gate) fails (30) before any size
        # check, and its code/notice surface instead of a size verdict.
    ...
    def test_tampered_manifest_surfaces_gate_error(self, package):
        # The single history load is the integrity gate: corrupting a generation manifest so it no longer matches its
        # own recorded hash makes the load fail, and size-only reports that manifest error instead of a size verdict.
...
class TestVerifyAscmhlSizes:
    """verify_ascmhl_sizes turns each recorded file into one ok / missing / mismatch SizeCheckResult.

    It runs off the loaded MHLHistory, so generation grouping, rename resolution and child-history descent are the
    reference's, not re-derived. We drive it against real sealed packages; the two branches a clean package can't reach
    (a path escaping the root, the resolve->getsize race) are provoked with targeted stubs.
    """
    ...
    def test_missing_recorded_size_is_ok_not_a_failure(self, tmp_path):
        # ascmhl omits the optional <path size> for a 0-byte file. The spec gives absence no meaning, so an existing
    ...
    def test_missing_recorded_size_still_checks_existence(self, tmp_path):
        # A sizeless record is existence-checked, not blindly passed: if its file is gone it is still reported missing.
    ...
    def test_directory_and_child_reference_entries_are_skipped(self, package):
        # The recorded set includes the A/ directory hash; only real files are size-checked (as the reference's verify
        # reaches files only), so the directory entry never leaks into the results.
        assert set(sizes_for(package)) == {"top.txt", "A/a1.txt", "A/a2.txt"}

    def test_nested_child_history_files_are_covered(self, tmp_path):
        # A sub-folder sealed as its own ASC-MHL history, then wrapped by a parent seal. The parent's size verify must
        # descend into the child, matching the hash path and the reference's combined-history grouping.
        root = tmp_path / "pkg"
    ...
    def test_original_generation_size_wins(self, tmp_path):
        # Seal a.mov at 100 bytes, then grow it to 200 and re-seal (creating gen 2). The recorded ORIGINAL size is
        # gen1's 100, so the on-disk 200 must mismatch — proving we compare against the first generation, not gen2.
    ...
    def test_path_escaping_root_is_blocked(self, package, monkeypatch):
        # A manifest can't normally record an escaping path, so stub the recorded set to force the traversal guard.
