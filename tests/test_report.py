...
_render_report and _format_file_result are pure functions writing to a buffer, and
ManifestResult/FileResult are the model they render. The mhlver CLI run tests only
feed the renderer empty/all-OK results, so the per-status summaries, the Details
section, the Issues roll-up, and the size-only/existence-only verdict downgrades are
exercised here against hand-built ManifestResults.
"""
